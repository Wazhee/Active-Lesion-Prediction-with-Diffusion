...
class BBDMRunner(DiffusionBaseRunner):
    ...
    def loss_fn(self, net, batch, epoch, step, opt_idx=0, stage='train', write=True):
        ...
        # region_loss = region_fn(input_x, output_x)
        # # loss = loss + region_loss # increased loss
        # # loss = loss - region_loss # reduced loss
        # loss = region_loss # region only loss
        if write:
    ...
    def sample(self, net, batch, sample_path, stage='train'):
        ...
        samples, prediction = net.sample(x_cond, clip_denoised=self.config.testing.clip_denoised).to('cpu')
        image_grid = get_image_grid(prediction, grid_size, to_normal=self.config.data.dataset_config.to_normal)
        # save prediction
